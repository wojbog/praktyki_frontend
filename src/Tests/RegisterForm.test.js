import { act, fireEvent, getByDisplayValue, getByTestId, render} from '@testing-library/react'
...
describe("Proper registration", () => {
    ...
    describe("with invalid name", () => {
        it('doesnt call the createUser function and shows proper message', async () => {
            ...
            expect(warnings[1].textContent).toBe("Jedno lub więcej pól jest nieprawidłowych!")
    ...
    describe("with invalid city", () => {
        it('doesnt call the createUser function and shows proper message', async () => {
            ...
            const warnings = component.getAllByTestId('invalid-msg')
            expect(warnings.length).toBe(2)
            expect(warnings[0].textContent).toBe("Nazwa miasta jest nieprawidłowa!")
            expect(warnings[1].textContent).toBe("Jedno lub więcej pól jest nieprawidłowych!")
        })  
    })
    ...
    describe("with invalid post_code", () => {
        it('doesnt call the createUser function and shows proper message', async () => {
            ...
            const warnings = component.getAllByTestId('invalid-msg')
            expect(warnings.length).toBe(2)
            expect(warnings[0].textContent).toBe("Kod pocztowy musi być w formacie: __-___")
            expect(warnings[1].textContent).toBe("Jedno lub więcej pól jest nieprawidłowych!")
        })  
    })
    ...
    describe("with responseError", ()=>{
        it('shows proper message', async () => {
            ...
            expect(warnings.length).toBe(1)
            expect(warnings[0].textContent).toBe("Coś poszło nie tak, spróbuj ponownie później")
        })  
    })   
})